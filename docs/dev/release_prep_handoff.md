...
| `578ae1d` | (spec) | Spec tests for HFER, IMPRINT, NDD-family (NDD/GIN/LiNDDA), pretrained classifiers (ONCET/WVNT). Replaces stale `test_gin.py` and `test_wavenet.py` relics. 22 new tests; spec coverage now exists for every detector in `__all__`. |
...
dynasd/
  ...
  NDD.py, GIN.py, LiNDDA.py      # NDDBase subclasses
...
| `test_ndd_family_spec.py` | 9 | NDD/GIN/LiNDDA: non-negativity, determinism, anomaly response (3 properties × 3 models) |
