...
class NDDBase(DynaSDBase):
    ...
    def __init__(self, fs=256, w_size=1, w_stride=0.5, use_cuda=False, **kwargs):
        ...
        self.mrse = training_kwargs.get('mrse', False)
    ...
    def _aggregate_sequences_to_windows_mse(self, seq_results, X):
        ...
        for win_idx in range(nwins):
            ...
            if seq_mask.any():
                ...
                window_mse[win_idx] = np.sqrt(np.mean(seq_mse[seq_mask], axis=0))
    ...
    def _get_features(self, X):
        ...
        with torch.no_grad():
            for inputs, targets in dataloader:
                ...
                seq_mse = torch.mean((predictions - targets) ** 2, dim=1)
    ...
    def forward(self, X):
        ...
        # Time-index inference outputs (spec section 5): every
